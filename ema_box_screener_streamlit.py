    ...
    with tabs[i]:
        try:
            ...
            with c1:
                ...
                st.dataframe(
                    movers_to_dataframe(gainers, tv_prefix=tv_prefix),
            ...
            with c2:
                ...
                st.dataframe(
                    movers_to_dataframe(losers, tv_prefix=tv_prefix),
                    use_container_width=True, hide_index=True,
                    column_config={"Chart": st.column_config.LinkColumn("Chart", display_text="Open")},
                )
...
with tabs[-1]:
    try:
        ...
        with c1:
            st.subheader("Top Gainers")
            st.dataframe(
                movers_to_dataframe(us_gainers, is_stock=True),
                use_container_width=True, hide_index=True,
                column_config={"Chart": st.column_config.LinkColumn("Chart", display_text="Open")},
            )
        with c2:
            st.subheader("Top Losers")
            st.dataframe(
                movers_to_dataframe(us_losers, is_stock=True),
                use_container_width=True, hide_index=True,
                column_config={"Chart": st.column_config.LinkColumn("Chart", display_text="Open")},
            )
    except Exception as e:
